...
public interface TournamentTeamMapper {
    ...
    @Insert("INSERT INTO team_tournaments(id_team, id_tournament, is_confirmed) VALUES(#{id_team}::integer, #{id_tournament}::integer, false)")
    void joinTournament(@Param("id_team") int id_team, @Param("id_tournament") int id_tournament);

    @Update("UPDATE team_tournaments SET is_confirmed=true WHERE id_team=${id_team} AND id_tournament=${id_tournament}")
    void acceptTeam(@Param("id_team") int id_team, @Param("id_tournament") int id_tournament);

    @Delete("DELETE FROM team_tournaments WHERE id_team=${id_team} AND id_tournament=${id_tournament}")
